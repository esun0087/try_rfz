...
def lpmv(l, m, x):
    ...
    for i in range(m_abs+2, l+1):
        ...
        # Inplace speedup
        y = ((2*i-1) / (i-m_abs)) * x * y
        y = y - ((i+m_abs-1)/(i-m_abs)) * yold
        yold = tmp

    if m < 0:
        y = y * ((-1)**m / pochhammer(l+m+1, -2*m))
...
def tesseral_harmonics(l, m, theta=0., phi=0.):
    ...
    N = N * np.sqrt(2. / pochhammer(l-abs(m)+1, 2*abs(m)))
    Y = Y * N
    return Y
...
class SphericalHarmonics(object):
    ...
    def lpmv(self, l, m, x):
        ...
        if l - m_abs > 1:
            y = y - ((l+m_abs-1)/(l-m_abs)) * self.leg[(l-2, m_abs)]
        #self.leg[(l, m_abs)] = y
