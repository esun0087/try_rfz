...
class Predictor():
    ...
    def predict(self, a3m_fn, out_prefix, hhr_fn=None, atab_fn=None, window=150, shift=75):
        ...
        print("predict",  "xyz_t", xyz_t.shape, "t1d", t1d.shape, "t0d", t0d.shape, "msa", msa.shape)
        ...
        with torch.no_grad():
            ...
            if L > window*2:
                ...
                print ("td shape", "msa", msa.shape, "seq", seq.shape, "t1d", t1d.shape, \
                    "t0d", t0d.shape, "t2d", t2d.shape,"xyz", xyz_t.shape, "idx_pdb", idx_pdb.shape)

                with torch.cuda.amp.autocast():
                    logit_s, _, xyz, lddt = self.model(msa, seq, idx_pdb, t1d=t1d, t2d=t2d)
                print ("model over", "xyz", xyz.shape, "lddt", lddt.shape)
                prob_s = list()
